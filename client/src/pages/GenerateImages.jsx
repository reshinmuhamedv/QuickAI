...
const GenerateImages = () => {
  ...
  const style = [
    'Realistic', 'Anime', 'Ghibli', 'Cartoon', 'Fantasy', '3D', 'Portrait', 'Pencil',
  ]
  ...
  <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200' onSubmit={onSubmitHandler}>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4}
          ...
          placeholder='Describe what your image should look like...' required/>
        <p className='mt-6 text-sm font-medium'>Style</p>

        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {style.map((item) => (
            <span key={item} onClick={() => setSelectedStyle(item)} 
        ...
        from-[#00AD25] to-[#04FF25] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
          {loading ? 
