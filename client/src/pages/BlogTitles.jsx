...
const BlogTitles = () => {
  ...
  const Category = [
    'General', 'Technology', 'Health', 'Lifestyle', 'Education', 'Travel', 'Food', 'Entertainment',
  ]
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200' onSubmit={onSubmitHandler}>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {Category.map((item) => (
            <span key={item} onClick={() => setSelectedCategory(item)} 
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600]'>
        ...
        {!content ? 
          ...
          <div className='mt-3 max-h-124 h-full overflow-y-scroll text-sm text-slate-600'>
